...
const traerPersonas = async()=>{
    ...
    ol.innerHTML="";
    try{
        let res = await fetch("http://jsonplaceholder.typicode.com/users");//espera a que termine fetch//no me bloque el hilo principal
        if(!res.ok){
            let mensaje = res.statusText || "Se produjo un error";
            //throw Error("Error: "+res.status+"-"+mensaje);
